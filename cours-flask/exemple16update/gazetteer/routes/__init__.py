...
def lieu_update(place_id):
    """ Route permettant la MAJ d'un lieu

    :param place_id: Identifiant numérique du lieu
    """
    lieu = Place.query.get(place_id)
    if request.method == 'POST':
        lieu.place_nom = request.form.get("nom", lieu.place_nom)
        db.session.add(lieu)
        db.session.commit()
    return render_template("pages/pages_edit.html", nom="Gazetteer", lieu=lieu)